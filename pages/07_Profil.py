...
import numpy as np
from utils import *   # contient déjà prepare_figure_for_export et plotly_chart_with_local_export
import json
import re
...
if df_sm.empty:
    ...
    with col1:
        ...
        plotly_chart_with_local_export(
            fig_taille,
            caption=f"Basé sur {total} supermarchés. Les pourcentages sont calculés sur l'ensemble des supermarchés.",
            key="fig_taille"
        )
    with col2:
        ...
        # Place la légende en dessous, horizontalement, centrée
    ...
    with col_a:
        ...
        plotly_chart_with_local_export(
            fig_pres_socio,
            caption=f"Basé sur {total} supermarchés. Les pourcentages sont calculés par niveau.",
            key="fig_pres_socio"
        )
    with col_b:
        st.write("Par taille")
        ...
        plotly_chart_with_local_export(
            fig_pres_taille,
            caption=f"Basé sur {total} supermarchés. Les pourcentages sont calculés par taille.",
            key="fig_pres_taille"
        )
    ...
    if 'Chaine' in df_sm.columns:
        ...
        plotly_chart_with_local_export(
            fig_chaine,
            caption=f"Basé sur {total_chaines} supermarchés ayant une chaîne renseignée.",
            key="fig_chaine"
        )
    else:
    ...
    plotly_chart_with_local_export(
        fig_secteur,
        caption=f"Basé sur {total} supermarchés.",
        key="fig_secteur"
    )
    ...
    try:
        ...
        if df_sm['duree_sem'].notna().any():
            ...
            plotly_chart_with_local_export(
                fig_ouv,
                caption=f"Basé sur {len(ouv_counts)} supermarchés ayant une heure d'ouverture renseignée.",
                key="fig_ouv"
            )
            ...
            plotly_chart_with_local_export(
                fig_duree,
                caption="Basé sur les supermarchés ayant des horaires complets.",
                key="fig_duree"
            )
    except Exception:
